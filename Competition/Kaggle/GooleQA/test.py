...
def conver_to_transformer_data(question_title, question_body, answer, tokenizer, max_length):
    q_input = tokenizer.encode_plus(question_title + ' ' + question_body, None, add_special_tokens=True,
                                    max_length=max_length)
    a_input = tokenizer.encode_plus(question_body + ' ' + answer, None, add_special_tokens=True, max_length=max_length)

    q_input_ids = q_input['input_ids']
    q_mask = q_input['special_tokens_mask']
    ...
    a_mask = a_input['special_tokens_mask']
...
tmp = conver_to_transformer_data(df_train['question_title'][0], df_train['question_body'][0], df_train['answer'][0],
...
def compute_input_array(df, columns, tokenizer, max_length):
    ...
    for _, i in tqdm(df[columns].iterrows()):
        ...
        q_input_ids, q_mask, q_token_type_ids, a_input_ids, a_mask, a_token_type_ids = conver_to_transformer_data(t, q,
                                                                                                                  a,
                                                                                                                  tokenizer,
                                                                                                                  max_length)
...
def model():
    ...
    config = BertConfig.from_pretrained(BERT_PATH + '/tokenizer_config.json')
    bert = TFBertModel.from_pretrained(BERT_PATH + '/tf_model.bin', config=config)
    '''
    BERT:
