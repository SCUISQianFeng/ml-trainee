...
class CaptionModel(nn.Module):
    def __init__(self, opt: Config, word2ix, ix2word):
        ...
        self.rnn = nn.LSTM(input_size=opt.embedding_dim, hidden_size=opt.rnn_hidden, num_layers=opt.num_layers)
        self.classifier = nn.Linear(in_features=opt.rnn_hidden, out_features=len(word2ix))
        self.embedding = nn.Embedding(num_embeddings=len(word2ix), embedding_dim=opt.embedding_dim)
