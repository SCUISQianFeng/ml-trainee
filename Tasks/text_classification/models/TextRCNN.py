...
class Model(nn.Module):
    def __init__(self, config: Config):
        ...
        self.fc = nn.Linear(in_features=config.hidden_size * 2 + config.embed, out_features=config.num_classes)

    def forward(self, x):
        x, _ = x
        embed = self.embedding(x)  # [batch_size, seq_len, embeding]=[64, 32, 64]
        out, _ = self.lstm(embed)  # out->[batch_size, seq_len, hidden_size * 2]
        out = torch.cat((embed, out), 2)  # short cut
        out = F.relu(out)
        out = out.permute(0, 2, 1)  # 将维度换位 【0,1,2】 -》 【0，2,1】
