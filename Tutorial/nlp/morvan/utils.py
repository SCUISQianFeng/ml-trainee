...
class DateData:
    def __init__(self, n):
        ...
        self.date_cn = []
        self.date_en = []
        for tiemstamp in range(143835585, 2043835585, n):
            date = datetime.datetime.fromtimestamp(tiemstamp)
            self.date_cn.append(date.strftime('%y-%m-%d'))
            self.date_en.append(date.strftime('%d/%b/%Y'))
        # i.split("/")[1] : 取月份的值
        self.vocab = set([str(i) for i in range(0, 10)] + ["-", "/", "<GO>", "<EOS>"] + [i.split("/")[1] for i in self.date_en])
        self.v2i = {value: index for index, value in enumerate(sorted(list(self.vocab)), start=1)}
        self.v2i['<PAD>'] = PAD_ID
        self.vocab.add('<PAD>')
        self.i2v = {index: value for value, index in self.v2i.items()}
        self.x, self.y = [], []

        for cn ,en in zip(self.date_cn, self.date_en):
            self.x.append([self.v2i[v] for v in cn])  # self.x just a list of index
    ...
    def sample(self, n=64):
        ...
        decoder_fen = np.full((len(bx), ), fill_value=by.shape[1] - 1, dtype=np.int32)
        return bx, by, decoder_fen


    def idx2str(self, idx: list):
        x = []
        for i in idx:
            x.append(self.i2v[i])
            if i == self.end_token:
