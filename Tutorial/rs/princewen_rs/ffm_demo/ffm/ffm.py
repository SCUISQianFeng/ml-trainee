...
class FFM:
    ...
    def train(self, dataset):
        ...
        for step in range(config.num_epoch):
            ...
            for iteration in range(num_iter_one_epoch):
                ...
                sys.stdout.write("\r")
                sys.stdout.write(
                    "=> [INFO] Process {:.0%} in Epoch {:d}: [Train] log-loss: {:.5f} one iter {:.1f} sec <= \r".format(
            ...
            if dataset.va_filename is not None:
                ...
                try:
                    ...
                    print(
                        "=> [INFO] STEP {}, [Val] val_loss: {:.5f}, val_auc: {:.3f} one epoch in {:.1f} sec <=".format(
    ...
    def _loss_func(self, pred, label):
        ...
        with tf.name_scope("l2_loss"):
            # l2 normalization
            regularizer = tf2.keras.regularizers.L1L2(l2=config.l2_norm)
            reg_term = tf2.contrib.layers.apply_regularization(regularizer, weights_list=None)
